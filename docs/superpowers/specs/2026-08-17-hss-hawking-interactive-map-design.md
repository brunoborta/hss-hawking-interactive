...
type CategoryId =
  ...
  | "self-destruct" | "black-box" | "pipe-lever-blue" | "pipe-lever-red" | "weapon"
...
- CSS grid with `grid-auto-flow: column` and 3 rows, so 11 categories form 4 columns (3+3+3+2); a new category simply starts a new column. Icon left, label right.
